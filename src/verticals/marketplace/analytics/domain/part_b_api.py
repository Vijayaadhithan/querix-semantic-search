...
def process_part_b(data):
    ...
    for path in api["execution_path"].unique():
        ...
        path_latency[path] = {
            ...
            "p95": round(float(path_data.quantile(0.95)), 1)
            if len(path_data) > 1
            else round(float(path_data.max()), 1),
    ...
    results["q25_det_vs_sem"] = {
        "comparison": {
            "deterministic_filter": {
                "count": int(len(det)),
                "avg_latency": round(float(det["duration_ms"].mean()), 1)
                if len(det) > 0
                else 0,
                "avg_results": round(float(det["total_results"].mean()), 1)
                if len(det) > 0
                else 0,
                "avg_tokens": round(float(det["total_tokens"].mean()), 1)
                if len(det) > 0
                else 0,
            },
            "semantic": {
                "count": int(len(sem)),
                "avg_latency": round(float(sem["duration_ms"].mean()), 1)
                if len(sem) > 0
                else 0,
                "avg_results": round(float(sem["total_results"].mean()), 1)
                if len(sem) > 0
                else 0,
                "avg_tokens": round(float(sem["total_tokens"].mean()), 1)
                if len(sem) > 0
                else 0,
            },
            "direct_semantic": {
                "count": int(len(dsem)),
                "avg_latency": round(float(dsem["duration_ms"].mean()), 1)
                if len(dsem) > 0
                else 0,
                "avg_results": round(float(dsem["total_results"].mean()), 1)
                if len(dsem) > 0
                else 0,
                "avg_tokens": round(float(dsem["total_tokens"].mean()), 1)
                if len(dsem) > 0
                else 0,
            },
        },
    ...
    results["q26_token_consumption"] = {
        "input_tokens": {
            "total": int(api["input_tokens"].sum()),
            "avg": _rounded_stat(api["input_tokens"].mean()),
        },
        "output_tokens": {
            "total": int(api["output_tokens"].sum()),
            "avg": _rounded_stat(api["output_tokens"].mean()),
        },
        "thought_tokens": {
            "total": int(api["thought_tokens"].sum()),
            "avg": _rounded_stat(api["thought_tokens"].mean()),
        },
        "total_tokens": {
            "total": int(api["total_tokens"].sum()),
            "avg": _rounded_stat(api["total_tokens"].mean()),
        },
        "labels": ["Input Tokens", "Output Tokens", "Thought Tokens"],
        "values": [
            int(api["input_tokens"].sum()),
            int(api["output_tokens"].sum()),
            int(api["thought_tokens"].sum()),
        ],
    ...
    operation_tokens = defaultdict(Counter)
    model_stats = defaultdict(
        lambda: {"count": 0, "total_latency": 0, "success": 0, "failure": 0}
    )
    ...
    for attempts in api["parsed_attempts"]:
        ...
        for attempt in attempts:
            ...
            token_group = operation_tokens[_operation_group(operation)]
            token_group["attempts"] += 1
            token_group["api_calls"] += _nonnegative_int(attempt.get("api_calls"))
            reported = False
            for token_name in (
                "input_tokens",
                "output_tokens",
                ...
                token_value = _nonnegative_int(attempt.get(token_name))
                token_group[token_name] += token_value
                reported = reported or token_value > 0
            if reported:
                token_group["attempts_with_reported_tokens"] += 1
    ...
    for p, s in provider_stats.items():
        provider_reliability[p] = {
            ...
            "failure": s["failure"],
            "fallback": s["fallback"],
            "success_rate": round(s["success"] / s["count"] * 100, 1)
            if s["count"] > 0
            else 0,
            "avg_latency": round(s["total_latency"] / s["count"], 1)
            if s["count"] > 0
            else 0,
        }
    ...
    # Q93: Separate provider-reported token totals for the model planner and
    # reranker. Zero is retained rather than estimated because some providers
    # do not expose token usage for every operation.
    token_fields = (
        "attempts",
        "api_calls",
    ...
    operation_token_data = {
        group: {
            name: int(operation_tokens[group].get(name, 0)) for name in token_fields
        }
        for group in ("llm", "reranker", "embedding")
    }
    results["q93_operation_token_usage"] = {
        "data": operation_token_data,
        "labels": ["LLM", "Reranker", "Embedding"],
        "values": [
            operation_token_data[group]["total_tokens"]
            for group in ("llm", "reranker", "embedding")
        ],
        "title": "Reported Tokens and Calls by Operation",
        "chart_type": "comparison_table",
        "note": (
            "Tokens are provider-reported. Zero can mean no token usage or "
            "that the provider did not return token telemetry; attempts and "
            "API calls remain authoritative."
        ),
    }
    ...
    model_latency = dict(
        sorted(model_latency.items(), key=lambda x: x[1]["count"], reverse=True)
    )
    ...
    results["q34_estimated_cost"] = {
        ...
        "cost_per_search": round(estimated_cost / total_requests * 1000, 4)
        if total_requests > 0
        else 0,
    ...
    merged["query_length"] = (
        merged["query_text"].fillna("").apply(lambda x: len(x.split()))
    )
    length_results = merged.groupby("query_length")["total_results"].mean()
    results["q39_length_vs_results"] = {
        "labels": [
            str(int(query_length)) for query_length in length_results.index[:15]
        ],
    ...
    results["q40_avg_api_calls"] = {
        ...
        "values": [
            int(api["api_call_count"].value_counts().sort_index()[k])
            for k in sorted(api["api_call_count"].value_counts().index)
        ],
    ...
    results["q41_multi_attempt"] = {
        "count": multi_attempt,
        "percentage": (
            round(multi_attempt / total_requests * 100, 1) if total_requests else 0
        ),
    ...
    results["q44_voyage_rate_limit"] = {
        ...
        "failure_rate": round(voyage_fail / len(voyage_events) * 100, 1)
        if voyage_events
        else 0,
        "failure_reasons": dict(
            Counter(e["failure_reason"] for e in voyage_events if e["failure_reason"])
        ),
    ...
    for idx, row in api.iterrows():
        ...
        for attempt in attempts:
            if (
                ...
                and attempt.get("operation") == "query_planning"
            ):
    ...
    if qp_latencies and total_latencies_for_qp:
        qp_pct = [
            round(qp / total * 100, 1)
            for qp, total in zip(qp_latencies, total_latencies_for_qp)
            if total > 0
        ]
        ...
        results["q46_query_planning_bottleneck"] = {
            "title": "Query Planning Bottleneck",
            "chart_type": "stat",
            "avg_qp_latency": 0,
        }
