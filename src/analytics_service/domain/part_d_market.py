...
def process_part_d(data):
    ...
    for cid, name in cat_names.items():
        ...
        top_city_id = cat_ads['city_id'].value_counts().index[0] if len(cat_ads) > 0 else None
    ...
    ads_city_state = ads.merge(location[['id', 'state_id']].rename(columns={'id': 'city_id'}),
                                on='city_id', how='left', suffixes=('', '_loc'))
    state_col = 'state_id_loc' if 'state_id_loc' in ads_city_state.columns else 'state_id'
    ...
    top_combos = ads.groupby(['category_id', 'city_id']).size().sort_values(ascending=False).head(30)
    combo_list = []
    ...
    active_monthly = ads[ads['deleted_at'].isna()].groupby('listing_month').size().tail(36)
    ...
    recent_users = users[users['created_at_parsed'] >= pd.Timestamp.now() - pd.Timedelta(days=365)]
    ...
    ads['engagement'] = (ads['actual_view_count'].fillna(0) +
                          ads['total_favorite'].fillna(0) * 5 +
                          ads['total_like'].fillna(0) * 3)
    top_engaged = ads.nlargest(15, 'engagement')[['title', 'engagement', 'actual_view_count', 'total_favorite', 'total_like']]
    results['q82_engagement'] = {
        'data': [{'title': str(r['title'])[:60],
    ...
    if 'created_at_parsed' not in users.columns:
        users['created_at_parsed'] = pd.to_datetime(users['created_at'], errors='coerce')
    merged = first_ad.merge(users[['id', 'created_at_parsed']].rename(columns={'id': 'user_id', 'created_at_parsed': 'reg_date'}),
    ...
    results['q83_activation'] = {
        'labels': ['Same day', '1-7 days', '8-30 days', '31-90 days', '91-365 days', '365+ days'],
        'values': [activation_dist.get(k, 0) for k in ['Same day', '1-7 days', '8-30 days', '31-90 days', '91-365 days', '365+ days']],
