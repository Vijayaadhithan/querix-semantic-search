...
def process_part_c(data):
    ...
    platform_map = {'1': 'iOS', '2': 'Android', '3': 'Web', 1: 'iOS', 2: 'Android', 3: 'Web'}
    platform_labeled = {}
    ...
    results['q55_ads_per_user'] = {
        ...
        'values': [apu_dist.get(k, 0) for k in ['1 ad', '2-3 ads', '4-5 ads', '6-10 ads', '11-20 ads', '20+ ads']],
    ...
    verified_count = int((users['is_verified'] == 1).sum() + (users['is_verified'] == '1').sum())
    ...
    users['days_since_last_activity'] = (pd.Timestamp.now() - users['updated_at_parsed']).dt.days
    ...
    results['q57_retention'] = {
        'labels': ['Active (≤30 days)', 'Recent (31-90 days)', 'Dormant (91-180 days)',
                   'Inactive (181-365 days)', 'Churned (>365 days)'],
        'values': [retention_dist.get(k, 0) for k in ['Active (≤30 days)', 'Recent (31-90 days)',
                   'Dormant (91-180 days)', 'Inactive (181-365 days)', 'Churned (>365 days)']],
    ...
    ads_by_subcat = ads.groupby('category_id').size().sort_values(ascending=False).head(20)
    results['q59_top_subcategories'] = {
        'labels': [subcat_names.get(int(sid), f'ID:{sid}') for sid in ads_by_subcat.index],
    ...
    ordered_labels = [f"₹{fee_bins[i]}-{fee_bins[i+1]}" for i in range(len(fee_bins)-1)] + [f"₹{fee_bins[-1]}+"]
    ...
    # Q61: Negotiable %
    negotiable = int((ads['is_rent_negotiable'] == 1).sum() + (ads['is_rent_negotiable'] == '1').sum())
    ...
    status_map = {'1': 'Active', '8': 'Active', '0': 'Inactive', '2': 'Pending', '3': 'Rejected',
                  '4': 'Expired', '5': 'Deleted', '6': 'Blocked', '7': 'Draft',
                  1: 'Active', 8: 'Active', 0: 'Inactive', 2: 'Pending', 3: 'Rejected'}
    ...
    top_viewed = ads.nlargest(10, 'actual_view_count')[['title', 'actual_view_count']].to_dict('records')
    top_fav = ads.nlargest(10, 'total_favorite')[['title', 'total_favorite']].to_dict('records')
    top_liked = ads.nlargest(10, 'total_like')[['title', 'total_like']].to_dict('records')
    results['q63_top_ads'] = {
        'top_viewed': [{'title': str(r.get('title', ''))[:60], 'value': int(r.get('actual_view_count', 0))} for r in top_viewed],
        'top_favorited': [{'title': str(r.get('title', ''))[:60], 'value': int(r.get('total_favorite', 0))} for r in top_fav],
        'top_liked': [{'title': str(r.get('title', ''))[:60], 'value': int(r.get('total_like', 0))} for r in top_liked],
        'title': 'Top Performing Ads',
        'chart_type': 'tables'
    }

    # Q64: City-wise ad density
    city_state = location.set_index('id')[['city', 'state_id', 'price']].to_dict('index')
    city_density = []
    for cid, count in ads_by_city.head(30).items():
        ...
        city_density.append({
            ...
            'state_id': int(info.get('state_id', 0)) if not pd.isna(info.get('state_id', 0)) else 0,
        })
    ...
    if len(premium_ads) > 0:
        ...
        results['q68_premium_by_category'] = {
            'labels': [], 'values': [], 'title': 'Premium Ads by Category', 'chart_type': 'bar'
        }
