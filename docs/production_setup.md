...
Query models use the `groq:` or `google:` configuration prefixes. The default
planner order is `groq:openai/gpt-oss-20b`, then
`google:gemini-3.1-flash-lite`. An environment may append one or more
`google:gemma-*` fallbacks through `QUERY_EXTRACT_MODELS`. Each provider attempt
...
minutes. `/api/v1/ready` rechecks loaded tenant services and remembers tenant
engine load failures without loading or evicting inactive tenant engines.
Authenticated tenant health performs the detailed tenant dependency check.
...
For low-traffic production hosts, install the periodic local search-path
warm-up timer documented in
[Production commands](production_commands.md#periodic-local-search-path-warm-up).
